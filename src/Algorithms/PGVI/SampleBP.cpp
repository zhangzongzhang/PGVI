...
{
	//constructor
	void SampleBP::setup(SharedPointer<MOMDP> _problem, PGVI* _solver)
	{
	...
	list < cacherow_stval >SampleBP::sample (cacherow_stval currIndexRow, unsigned int currentRoot)
	{
		...
		//if (trialTargetPrecision == -1)//need for hsvi and fixed precision bp
		{
			...
			double newDelta;
			if (solver->solverParams->timeoutSeconds > 0)
			{
				newDelta = 0.5 * (solver->solverParams->timeoutSeconds - solver->elapsed) / solver->solverParams->timeoutSeconds;
			}
			else
			{
				newDelta = 0.5 * (1000.0 - solver->elapsed) / 1000.0;
			}
			solver->packingSetManager->resetDelta(newDelta); 
		...
		else
		{
			...
			int numStatesObs = solver->problem->XStates->size();
			double maxReward = -99e+20;
			FOR (sval, numStatesObs)
			{
				double reward = solver->problem->rewards->getMatrix(sval)->getMaxAbsoluteValue();
				if (reward > maxReward)
				{
					maxReward = reward;
				}
			}
			double deltaForConvergence = pow(1 - solver->problem->getDiscount(), 2.0) * trialTargetPrecisionArr[currentRoot] /
				(2 * solver->problem->getDiscount() * maxReward);

			//to control the width of the search tree and guarantee the convergence after performing at most h^2 * C * |A| * |Z| point-based updates.
			newIndexRow = solver->packingSetManager->getTheClosestBeliefInPacking(deltaForConvergence, newIndexRow, depthArr[currentRoot] + 1, solver->beliefCacheSet);
		  ...
		  if(NULL != Qax) {
			...
			//FOR (o, Qax.getNumOutcomes ())
			{
				...
				if (NULL != e)
				{
					...
					double distance = solver->packingSetManager->calculateWeightedMinimumDistanceBetweenBeliefAndPacking(sn.cacheIndex, depthArr[currentRoot] + 1, solver->beliefCacheSet, solver->numBackups);
					//the distance between the next belief and the corresponding packing
