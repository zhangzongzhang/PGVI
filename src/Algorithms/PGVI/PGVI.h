...
#define CHECK_INTERVAL 50
	...
	class PGVIAlphaPlaneTuple :public Tuple
	{
	public:
	...
	class PGVI :	public PointBasedAlgorithm
	{
	private:
	...
	public:
		...
		PGVI(SharedPointer<MOMDP> problem, SolverParams * solverParams);
		virtual ~PGVI(void);
		...
		{
			PGVI *pgviSolver = (PGVI *)solver;
			int stateidx = belief->sval;
			int row = node->cacheIndex.row;
			int timeStamp = pgviSolver->numBackups;

			// PGVI Bin Manager related
			pgviSolver->binManagerSet->binManagerSet[stateidx]->binManagerDataTable.set(row).binned = false;
			...
			pgviSolver->upperBoundSet->set[stateidx]->dataTable->set(row).UB_ACTION = 0;
			

			list<SharedPointer<AlphaPlane> >* alphas = new list<SharedPointer<AlphaPlane> >();
			pgviSolver->lowerBoundSet->set[stateidx]->dataTable->set(row).ALPHA_PLANES= alphas;

			// TODO:: fix it
			SharedPointer<AlphaPlane>alpha = pgviSolver->lowerBoundSet->getValueAlpha(belief);
			//REAL_VALUE lbVal = pgviSolver->lowerBoundSet->getValue(belief);
			REAL_VALUE lbVal = inner_prod(*alpha->alpha, *belief->bvec);
			//REAL_VALUE lbVal = bounds->getLowerBoundValue(b_s, &alpha);



			PGVIAlphaPlaneTuple *dataAttachedToAlpha = (PGVIAlphaPlaneTuple *)(alpha->solverData);
			//TODO: dataAttachedToAlpha->maxMeta



			REAL_VALUE ubVal =pgviSolver->upperBoundSet->getValue(belief);
			...
			pgviSolver->lowerBoundSet->set[stateidx]->dataTable->set(row).ALPHA_TIME_STAMP = timeStamp;
		...
		{
			PGVIAlphaPlaneTuple *attachedData = (PGVIAlphaPlaneTuple *)alpha->solverData;
