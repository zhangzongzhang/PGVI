...
void PGVI::progressiveIncreasePolicyInteval(int& numPolicies)
{
	if (numPolicies == 0) 
	{
...
PGVI::PGVI(SharedPointer<MOMDP> problem, SolverParams * solverParams)
{
...
void PGVI::solve(SharedPointer<MOMDP> problem)
{
	try
	{
		...
		//cout << "finished calling initialize() in PGVI::solve()" << endl;
		...
		while(!stop)
		{
			...
			if (activeRoot == -1) 
			{
				FOR (r, globalroot.getGlobalRootNumSampleroots()) 
				{
					...
					if (NULL != eR) 
					{
						...
						packingSetManager->addBeliefsToPacking(sampledBeliefs, beliefCacheSet);
					}
				}
				sampledBeliefs.clear();
			} 
			else  
			{
				...
				if (((SampleBP *)sampleEngine)->newTrialFlagArr[activeRoot] == 1) 
				{
					...
					sampledBeliefs.reverse();
					packingSetManager->addBeliefsToPackingSet(sampledBeliefs, beliefCacheSet, numBackups);
					//packingSetManager->addBeliefsToPacking(sampledBeliefs, beliefCacheSet);
	...
	packingSetManager->printPackingSetSize();
	printf("Time used in PackingSetManager: %f\n", packingSetManager->getPackingSetTimerElapsed());
...
void PGVI::alwaysPrint()
{
	...
	cout.width(9);cout << left << packingSetManager->getUniquePackingSize();
...
void PGVI::logFilePrint(int index)
{
	//struct tms now;
	//float utime, stime;
	//long int clk_tck = sysconf(_SC_CLK_TCK);
...
void PGVI::writeIntermediatePolicyTraceToFile(int trial, double time, const string& outFileName, string problemName)
{
...
BeliefTreeNode& PGVI::getMaxExcessUncRoot(BeliefForest& globalroot) 
{
...
void PGVI::backup(BeliefTreeNode* node)
{
	upperBoundSet->backup(node);
	lowerBoundSet->backup(node);
}

void PGVI::initialize(SharedPointer<MOMDP> problem)
{
	...
	pruneEngine = new PGVIPrune(this);
	packingSetManager = new PackingSetManager();

}
void PGVI::initSampleEngine(SharedPointer<MOMDP> problem)
{
	sampleEngine->appendOnGetNodeHandler(&PGVI::onGetNode);
...
void PGVI::initializeUpperBound(SharedPointer<MOMDP> problem)
{
	...
	upperBoundSet->appendOnBackupHandler(&PGVI::onUpperBoundBackup);
	((BackupBeliefValuePairMOMDP*)upperBoundBackup)->boundSet = upperBoundSet;
}
void PGVI::initializeLowerBound(SharedPointer<MOMDP> problem)
{
	...
	lowerBoundSet->appendOnBackupHandler(&PGVI::onLowerBoundBackup);
	lowerBoundSet->appendOnBackupHandler(&PGVIPrune::onLowerBoundBackup);
	((BackupAlphaPlaneMOMDP* )lowerBoundBackup)->boundSet = lowerBoundSet;
}

void PGVI::initializeBounds(double _targetPrecision)
{
...
cacherow_stval PGVI::backup(list<cacherow_stval> beliefNStates)
{
	...
	if(nextRowNState.row== -1)
	{
		...
		cout << "In PGVI::backup( )" << endl;
	}
	return nextRowNState;
}//end method: backup


cacherow_stval PGVI::backupLBonly(list<cacherow_stval> beliefNStates){
	//decide the order of backups 
	cacherow_stval rowNState, nextRowNState;
	nextRowNState.row = -1;
	...
	if(nextRowNState.row== -1){
		printf("Error: backup list empty\n");
		cout << "In PGVI::backupLBonly( )" << endl;
	}
	return nextRowNState;
}//end method: backup
...
cacherow_stval PGVI::backup(cacherow_stval beliefNState)
{

	unsigned int stateidx = beliefNState.sval;
	int row = beliefNState.row;

	//cout << "in PGVI::backup(), beliefNState.sval : " << beliefNState.sval << " beliefNState.row : " << beliefNState.row << endl;
...
cacherow_stval PGVI::backupLBonly(cacherow_stval beliefNState){
...
void PGVI::writePolicy(string fileName, string problemName)
{
	writeToFile(fileName, problemName);
}

void PGVI::writeToFile(const std::string& outFileName, string problemName) 
{
	lowerBoundSet->writeToFile(outFileName, problemName);

}

void PGVI::printHeader(){
    cout << endl;
    printDivider();
	cout << " Time   |#Trial |#Backup |LBound    |UBound    |Precision  |#Alphas |#Beliefs  |#Packing  " << endl;
    printDivider();
}

void PGVI::printDivider(){
    cout << "----------------------------------------------------------------------------------------" << endl;
}
