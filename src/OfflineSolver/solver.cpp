		...
		case S_PGVI:
			{
				...
				pgviSolver = new PGVI(problem, p);

				lbBackup->problem = problem;
				pgviSolver->lowerBoundBackup = lbBackup;

				((BackupAlphaPlaneMOMDP* )(pgviSolver->lowerBoundBackup))->solver = pgviSolver;

				ubBackup->problem = problem;
				pgviSolver->upperBoundBackup = ubBackup;
				solver = pgviSolver;
			}
			break;
