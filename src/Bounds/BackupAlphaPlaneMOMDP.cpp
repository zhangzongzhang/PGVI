...
SharedPointer<AlphaPlane> BackupAlphaPlaneMOMDP::backup(BeliefTreeNode* cn)
{
	...
	// TODO:: migrate below code to PGVI::onLowerBoundBackup
	...
	if(alphas->size()>0)
	{
		...
		PGVIAlphaPlaneTuple *tempTuple = (PGVIAlphaPlaneTuple *)frontAlpha->solverData;
		tempTuple->certed--;
		alphas->pop_front();
	}
	((PGVIAlphaPlaneTuple *)newPlaneP->solverData)->certed ++;
